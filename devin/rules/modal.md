...
description: When code imports `modal`, the project deploys to Modal (modal.com), or the task involves Modal Apps, Functions, Cls, Servers, Sandboxes, Images, Volumes, Secrets, GPUs, web endpoints, or the `modal` CLI
---
...
2. **SDK source**: the `modal-labs/modal` monorepo (private; Kathir has access). Relevant paths:
   ...
   Getting it: use an existing clone in the workspace if there is one. Otherwise `git clone --depth 1 --filter=blob:none --sparse https://github.com/modal-labs/modal && git -C modal sparse-checkout set client/py examples frontend/src/lib/docs/guide` (the full repo is large). Never modify or push to it unless asked.
   If your GitHub integration can't reach it, use the public mirrors: `modal-labs/modal-client` (the `client/` dir) and `modal-labs/modal-examples` (the `examples/` dir).
